    ...
    return (
        <Box className={`opacity-transition add-new-habit-backdrop backdrop`}>
            <Card component="form" onSubmit={updateHabit} className={`add-new-habit-form scale-in`}>
                ...
                {(habitInputs.goalMode || props.detailedHabit?.goalId) && 
                <DatePicker 
                ...
                />}
                {(habitInputs.goalMode || props.detailedHabit?.goalId) && 
                <TextField value={habitInputs.goalTitle} onChange={(event)=>{habitInputsHandler(event,'goalTitle')}} label='Goal Title'  className="add-new-habit-goal-title" multiline required />}
                <Box className="add-new-habit-buttons">
                    <Button variant="outlined" type='button' className='button' onClick={()=>{props.setDetailedItem();props.returnToHabits()}}>Go Back</Button>
